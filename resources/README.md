...
The databricks CLI bundles top level is defined by file `btafur_mlops_project/databricks.yml`.
...
ML Resource Configurations in this directory:
 - model workflow (`btafur_mlops_project/resources/model-workflow-resource.yml`)
 - batch inference workflow (`btafur_mlops_project/resources/batch-inference-workflow-resource.yml`)
 - monitoring workflow (`btafur_mlops_project/resources/monitoring-workflow-resource.yml`)
 - feature engineering workflow (`btafur_mlops_project/resources/feature-engineering-workflow-resource.yml`)
 - model definition and experiment definition (`btafur_mlops_project/resources/ml-artifacts-resource.yml`)
...
|-------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------------|-------------------------------------|------------------------------------------------|
| dev         | The `dev` deployment target is used by ML engineers to deploy ML resources to development workspace with `dev` configs. The config is for ML project development purposes.                                                           | dev workspace        | dev-btafur-mlops-project-model     | /dev-btafur-mlops-project-experiment     |
| staging     | The `staging` deployment target is part of the CD pipeline. Latest main content will be deployed to staging workspace with `staging` config.                                                             | staging workspace    | staging-btafur-mlops-project-model | /staging-btafur-mlops-project-experiment |
| prod        | The `prod` deployment target is part of the CD pipeline. Latest release content will be deployed to prod workspace with `prod` config.                                                                      | prod workspace       | prod-btafur-mlops-project-model    | /prod-btafur-mlops-project-experiment    |
| test        | The `test` deployment target is part of the CI pipeline. For changes targeting the main branch, upon making a PR, an integration test will be triggered and ML resources deployed to the staging workspace defined under `test` deployment target. | staging workspace    | test-btafur-mlops-project-model    | /test-btafur-mlops-project-experiment    |
...
2. Complete the `TODO` in `btafur_mlops_project/databricks.yml` to add the dev workspace URI under `targets.dev.workspace.host`.
...
Following this, update the batch_inference_job base parameters in `btafur_mlops_project/databricks-resource/batch-inference-workflow-resource.yml` to pass
...
To get started, open `btafur_mlops_project/resources/batch-inference-workflow-resource.yml`.  The file contains the ML resource definition of a batch inference job, like:
...
resources:
  jobs:
    batch_inference_job:
      name: ${bundle.target}-btafur-mlops-project-batch-inference-job
      tasks:
...
The example above defines a Databricks job with name `${bundle.target}-btafur-mlops-project-batch-inference-job`
that runs the notebook under `btafur_mlops_project/deployment/batch_inference/notebooks/BatchInference.py` to regularly apply your ML model for batch inference. 
...
We specify a `batch_inference_job` under `resources/jobs` to define a databricks workflow with internal key `batch_inference_job` and job name `{bundle.target}-btafur-mlops-project-batch-inference-job`.
...
`test-btafur-mlops-project-batch-inference-job`. During the deployment of the `staging` environment config, the job name will be
`staging-btafur-mlops-project-batch-inference-job`.
